...
> ### 바텀 네비게이션 (activity_main.xml, MainActivity.kt) (하은영)
  - 바텀 네비게이션을 구현하기 위해 BottomNavigationView 및 화면이 표시되는 FrameLayout 사용.
  - home, my, setting의 총 세 가지의 옵션: 각각 프래그먼트로 개발하게 함.
     
> ### 메인 페이지 - 홈 프라그먼트 (fragment_home.xml, HomeFragment.kt) (하은영)
  - 안심동행 신청 버튼: 안심 동행을 신청하기 위한 아이 인적사항 입력 페이지를 라우팅.
  - 동행 상황 바로 가기 버튼: 안심 동행 현황에 대해 볼 수 있는 페이지를 라우팅.
  - SharedPreferences 기능을 사용: 안심동행이 진행 중일 때 안심동행 신청을 누르거나, 안심동행이 진행 중이지 않을 때 동행 상황 바로가기를 누르지 못하게(토스트 메시지) 구현.
  - 예치금 바로가기: 예치금을 입금할 수 있는 안내 메시지가 있는 페이지를 라우팅.
  - 로그아웃: 로그아웃을 하게 되면 로그인 시에 저장되어 있던 SharedPreferences의 아이디를 지워 로그아웃 기능을 구현.
...
> ### 예치금 결제하기 (PaymentActivity.kt) (하은영)
  - 결제 api, 가상계좌의 방법 모색 -> 실제로 사용하려면 사업자 등록증 혹은 비용 문제 발생 -> 사용자가 관리자 계좌로 입금하게 되면, 관리자가 직접 예치금을 관리하는 방식으로 구상.
...
> ### 예치금 관리 페이지1(AdminDepositFragment.kt, AdminDepositManagementActivity.kt) (하은영)
  - RecyclerView를 사용하여 모든 사용자들의 아이디 및 이름 확인 가능.
  - 리스트 클릭 시, 해당 사용자의 예치금을 관리할 수 있는 페이지를 라우팅. -> 사용자의 아이디와 함께 서버에 데이터 요청 / 서버에서는 사용자의 아이디, 이름, 전화번호, 현재 예치금 정보를 넘겨줌.
  - SearchView를 사용하여, 아이디로 사용자를 검색할 수 있는 검색창 기능.

> ### 예치금 관리 페이지2 (하은영)
  - 사용자의 정보 (아이디, 이름, 전화번호) 확인 가능.
  - 예치금을 관리할 수 있는 기능.
